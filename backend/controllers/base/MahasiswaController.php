        ...
        try {
            if ($model->load($_POST)) {
                ...
                if ($img != NULL) {
                    ...
                    $path = Yii::getAlias("@backend/web/uploads/mahasiswa/foto/") . $model->foto;
                    $img->saveAs($path);
                }
                $model->save();
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load($_POST)) {
            ...
            if(empty($img)){
                ...
                $model->foto = $img;
                $model->upload();
                $model->foto = time().".".UploadedFile::getInstance($model,'foto')->getExtension();
            }
            $model->save();
...
// TODO: improve detection
    ...
    public function actionPay($id,$bln){
        $model = new DonasiRutin();
        $mhs = $this->findModel($id);
        $model->id_mahasiswa = Mahasiswa::findOne($id)->id;
        $model->jumlah = Mahasiswa::findOne($id)->jumlah_donasi;
        $model->bulan = $bln;
        $model->tahun = date("Y");
        $model->tanggal = date('d-M-Y');
        $model->fakultas = $mhs->fakultas;
        $model->jurusan = $mhs->jurusan;
        if($model->save()){
            return $this->redirect(Url::previous());
        }
